#!/usr/bin/env python3
"""
uav_tx.py -- AUTONOMOUS TX supervisor for the RZ/V2H UAV video+detection link.

Runs UNATTENDED on the board (systemd unit uav-tx.service). It never streams on
...
Run (foreground, for testing):   sudo python3 uav_tx.py
Run (production):                systemd unit uav-tx.service (see README)
...
class TxSupervisor:
            ...
            if "[app_m5 live]" in line:
                ...
                if m:
                    try:
                        ...
                        pass
            elif "[uav_enc] AQC" in line and "level=" in line:
                # Phase1 AQC ladder: uav_enc prints its CURRENT level + the actual
                ...
                #   [uav_enc] AQC level=L1 target=5000kbps (retries/s=.. sig=.. loss=..)
                d = {}
    ...
    def telemetry_loop(self):
        ...
        while self.running:
            ...
            # Phase1 AQC: uav_enc drives target-bitrate at runtime (the 4-level
    ...
    def _declare_camera_wedged(self, conn, why):
        ...
        process restarts (a board reboot restarts uav-tx fresh -> flag cleared)."""
    ...
    def run(self):
        ...
        self.evt.log("tx_boot", "uav_tx supervisor starting host=%s" % hostname)
