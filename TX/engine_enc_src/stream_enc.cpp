 ...
 * Built ONLY into the standalone uav_enc child (clean gst process).
 *
...
GQuark release_quark() {
    static GQuark q = g_quark_from_static_string("uav-stream-release-ctx");
    return q;
}
...
int stream_enc_init(const char* host, int port, int bitrate, int w, int h, int in_fps) {
     ...
     * TX-side half lives in uav_enc.cpp's REQ_IDR handler.
...
void stream_enc_enable_sei(void) {
    ...
    std::fprintf(stderr, "[stream_enc] SEI box embedding ENABLED (UUID=UAVBOXSEI01_DRPI)\n");
}
