 ...
 * no gst), so it can be linked into the uav_enc child WITHOUT pulling extra deps.
 *
 * ===========================================================================
 *  WHAT / WHY
 * ===========================================================================
...
private:
    ...
    int wr_reg(unsigned char reg, unsigned char val) {
        ...
        return (::write(fd_, b, 2) == 2) ? 0 : -1;
    }
    int rd_reg(unsigned char reg, unsigned char* val) {
        if (::write(fd_, &reg, 1) != 1) return -1;
        return (::read(fd_, val, 1) == 1) ? 0 : -1;
    }
    /* burst read n bytes starting at reg (IF_INC auto-increments the reg ptr). */
    int rd_burst(unsigned char reg, unsigned char* buf, int n) {
        if (::write(fd_, &reg, 1) != 1) return -1;
        return (::read(fd_, buf, n) == n) ? 0 : -1;
    }
