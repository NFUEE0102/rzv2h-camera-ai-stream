 ...
 * hysteresis timers from uav_enc.cpp's fb_thread() and drives it through a
 ...
 * parity (the 60->30 step) exactly as the uav_enc main loop implements it.
 ...
 *
 * The stepping + parsing code below is copied 1:1 from uav_enc.cpp (same
...
/* ---- ladder config (mirrors uav_enc.cpp defaults) ----
...
/* heal band (mirrors uav_enc.cpp): the UP path demands CLEARLY better than the
...
/* applied-level recorder (stub for uav_enc.cpp's aqc_apply_level, which does
 * stream_enc_set_bitrate + publishes fps_div to the frame loop). The
 * g_fps_br_comp scaling mirrors uav_enc.cpp's YOLO_AQC_FPSDIV_BR_COMP
...
/* ==== YOLO_AQC_LEVELS parser, copied 1:1 from uav_enc.cpp aqc_parse_levels
...
int main() {
    ...
    for (double tnow = 0.0; tnow <= T_END; tnow += DT) {
        ...
        /* ==== EXACT logic copied from uav_enc.cpp fb_thread ==== */
    ...
    /* ==== part 2: fps_div frame-drop parity (copied 1:1 from the uav_enc main
     ...
     * (see the caveat block in uav_enc.cpp). comp ON (YOLO_AQC_FPSDIV_BR_COMP
