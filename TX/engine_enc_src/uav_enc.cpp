/*
 * uav_enc.cpp -- standalone H.265 (HEVC) encoder child for app_yolox_cam (Design B).
 *
 ...
 *
 *   uav_enc <sockfd> <host> <port> <bitrate>
 */
 ...
 *
 * The PC (uav_view.py) still sends a 16-byte FbMsg ~1 Hz (loss + recv kbps);
...
#pragma pack(pop)
#define FB_MAGIC      0x55415646u  /* 'UAVF' (LE) */
#define FB_VER        1
 ...
 * defaults to 8/12 but uav_tx.sh passes the active profile's real k/n via
...
#define UAV_CAP_BUF_NUM 8
#define UAV_CAP_W       1920
#define UAV_CAP_H       1080
#define UAV_PAD_H       1920
#define UAV_FRAME_SIZE  (UAV_CAP_W * UAV_CAP_H * 2)   /* encoder reads this (top rows) */
#define UAV_BUF_SIZE    (UAV_CAP_W * UAV_PAD_H * 2)   /* mmngr alloc: frame + gray pad */

namespace {
struct ChildBuf { int dbuf_fd; uint32_t phys; uint32_t size; MMNGR_ID id; };
ChildBuf g_buf[UAV_CAP_BUF_NUM];
...
int alloc_buffers() {
    for (int i = 0; i < UAV_CAP_BUF_NUM; i++) {
        MMNGR_ID id; unsigned int phys = 0; void* virt = nullptr; int dfd = -1;
        int r = mmngr_alloc_in_user_ext(&id, UAV_BUF_SIZE, &phys, &virt,
                                        MMNGR_VA_SUPPORT, NULL);   /* non-cached */
        if (r != R_MM_OK || !virt) {
            std::fprintf(stderr, "[uav_enc] mmngr_alloc[%d] r=%d\n", i, r); return -1;
        }
        ...
        for (size_t k = 0; k + 3 < UAV_BUF_SIZE; k += 4) {
            p[k] = 114; p[k+1] = 128; p[k+2] = 114; p[k+3] = 128;
        }
        r = mmngr_export_start_in_user_ext(&id, UAV_BUF_SIZE, phys, &dfd, NULL);
        if (r != R_MM_OK || dfd < 0) {
            std::fprintf(stderr, "[uav_enc] mmngr_export[%d] r=%d\n", i, r); return -1;
        }
        g_buf[i].dbuf_fd = dfd; g_buf[i].phys = (uint32_t)phys;
        g_buf[i].size = (uint32_t)UAV_BUF_SIZE; g_buf[i].id = id;
    }
    return 0;
}
    ...
    while (*p && n < AQC_MAX_LEVELS) {
        ...
        if (end == p) {
            ...
            if (*p != ',' && *p != ' ') {
                ...
                std::fprintf(stderr,
                    "[uav_enc] AQC levels: unparsable entry '%.*s' skipped\n",
                    tl, tok);
        ...
        if (*p == '@') {                       /* optional fps suffix */
            ...
            if (e2 != p + 1) {
                if (fps == 30) fdiv = 2;       /* the only allowed step: 60->30 */
                    ...
                    "[uav_enc] AQC levels: '@%ld' on %ldkbps ignored (only @30 "
                    "supported; 30fps is the floor)\n", fps, v);
                p = e2;
            } else {                           /* '@' with no number after it */
                std::fprintf(stderr,
                    "[uav_enc] AQC levels: empty '@' on %ldkbps ignored\n", v);
                p++;
            }
        }
        ...
        if (*p && *p != ',' && *p != ' ') {
            int tl = 0; while (tok[tl] && tok[tl] != ',' && tl < 24) tl++;
            std::fprintf(stderr,
                "[uav_enc] AQC levels: trailing garbage in entry '%.*s' "
...
void* fb_thread(void* arg) {
    ...
    { char lad[160]; int off = 0;
      ...
      std::fprintf(stderr,
        "[uav_enc] AQC start level=L%d target=%dkbps@%dfps ladder=%s "
    ...
    if (inj_retries > -900 || inj_signal > -900 || inj_loss > -900)
        std::fprintf(stderr, "[uav_enc] AQC TEST-INJECT retries/s=%.1f sig=%.1f loss=%.2f%% "
    ...
    while (c->running.load()) {
        ...
        if (n == (ssize_t)sizeof(m) && m.magic == FB_MAGIC &&
            ...
            if (have_seq && m.seq <= last_seq) {
                ...
                else std::fprintf(stderr, "[uav_enc] feedback sender resync (seq %u<=%u)\n",
                                  m.seq, last_seq);
            }
            if (accept) {
                ...
                if (ondemand_idr && (fb_flags & FB_FLAG_REQ_IDR)) {
                    ...
                    if (last_idr_req_t == 0.0 || tnow_idr - last_idr_req_t >= idr_min_s) {
                        ...
                        std::fprintf(stderr, "[uav_enc] on-demand IDR requested (REQ_IDR)\n");
                    }
                }
            }
        }
        ...
        if (g_levels[lvl].fps_div >= 2 && tnow - last_probe_t >= 5.0) {
            last_probe_t = tnow;
            std::fprintf(stderr,
                "[uav_enc] AQC 30fps wire probe: L%d label=%dkbps enc_target=%dkbps "
        ...
        if (dwell_ok && lvl < g_n_levels - 1 && unhealthy_since > 0.0 &&
            ...
            std::fprintf(stderr,
                "[uav_enc] AQC level=L%d target=%dkbps@%dfps DOWN (retries/s=%.1f sig=%.1f "
         ...
         * step AND the post-congestion hold-down elapsed AND dwell satisfied ---- */
            ...
            std::fprintf(stderr,
                "[uav_enc] AQC level=L%d target=%dkbps@%dfps UP (retries/s=%.1f sig=%.1f "
...
int main(int argc, char** argv) {
    if (argc < 5) {
        std::fprintf(stderr, "usage: uav_enc <sockfd> <host> <port> <bitrate>\n");
        return 2;
    }
    ...
    std::fprintf(stderr, "[uav_enc] start sock=%d -> %s:%d br=%d\n", sock, host, port, bitrate);
    ...
    for (int i = 0; i < UAV_CAP_BUF_NUM; i++) {
        ...
        if (enc_send_buf(sock, bm, g_buf[i].dbuf_fd) != 0) {
            std::fprintf(stderr, "[uav_enc] send_buf[%d] failed\n", i); return 1;
        }
    }
    std::fprintf(stderr, "[uav_enc] sent %d capture buffers to app\n", UAV_CAP_BUF_NUM);

    if (stream_enc_init(host, port, bitrate, UAV_CAP_W, UAV_CAP_H, 60) != 0) {
        std::fprintf(stderr, "[uav_enc] stream_enc_init failed\n"); return 1;
    }
    ...
    if (sei_on) {
        ...
        std::fprintf(stderr, "[uav_enc] YOLO_SEI=1 -> SEI box embedding active\n");
    }
    ...
    else std::fprintf(stderr, "[uav_enc] WARN: AQC thread create failed -- fixed bitrate\n");
    ...
    while ((r = enc_recv_msg(sock, &is_box, &m, &bm, recs, SEI_BOX_MAX,
        ...
        if (m.index >= UAV_CAP_BUF_NUM) continue;
        ...
        stream_enc_push(g_buf[m.index].dbuf_fd, UAV_FRAME_SIZE, m.pts_ns,
                        child_on_release, ctx);   /* encode the top 1080 rows only */
    }
    std::fprintf(stderr, "[uav_enc] parent closed (r=%d), stopping\n", r);
