...
(uav_tx.py) reads it once at startup.
"""
import os
...
ENGINE_USER_HOME = os.environ.get("UAV_ENGINE_HOME", "/home/ubuntu")
...
ENGINE_CHILD = "uav_enc_m5"       # the RTP-streaming child app_m5 exec()s
...
#     (a reboot restarts uav-tx fresh and clears the flag).
CAM_PREP_TIMEOUT_S       = float(os.environ.get("UAV_CAM_PREP_TIMEOUT", "8"))
MIN_ENGINE_RESTART_S     = float(os.environ.get("UAV_MIN_ENGINE_RESTART", "4"))
CAM_WEDGE_FAIL_THRESHOLD = int(os.environ.get("UAV_CAM_WEDGE_FAIL_THRESHOLD", "2"))

# ---- stream parameters handed to the engine via env ----
STREAM_RTP_PORT  = 50010
STREAM_BBOX_PORT = 50012
# Phase1: start at the healthy L0 = 8 Mbps ceiling. uav_enc's AQC ladder picks the
...
TIME_SYNC_THRESHOLD_S = float(os.environ.get("UAV_TIME_SYNC_THRESHOLD", "2.0"))
