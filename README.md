...
`app_m5` (camera capture + DRP-AI inference) and `uav_enc_m5` (GStreamer
...
camera's capture buffers**, not the capture app. `uav_enc_m5` allocates the
...
```
TX/
  uav_tx.py              board-side supervisor: discovery, control-plane
  ...
  uav-tx.service           systemd unit for unattended board boot
  ...
  engine_enc_src/          uav_enc_m5: GStreamer H.265 encode + SEI + AQC
    uav_enc.cpp              entry point, encoder main loop, AQC controller
...
RX/
  uav_rx.py                ground-station supervisor: discovery, control-plane
                            state machine, auto-reconnect/retry, telemetry
  uav_view.py               the actual viewer: GStreamer decode, SEI
...
because `app_m5` and `uav_enc_m5` are separate binaries with separate build
...
`user_data_unregistered`, UUID `UAVBOXSEI01_DRPI`):
...
Python side (`uav_tx.py`, `uav_rx.py`, `uav_view.py`) needs GStreamer's
...
- The AQC bitrate-vs-actual-framerate compensation (`YOLO_AQC_FPSDIV_BR_COMP`
  in `uav_enc.cpp`) has a documented open question about whether the
...
- `uav_view.py`'s GL-accelerated horizon-rotation path
...
- This is an extract of the camera/AI/encode/managed-network-transport
  ...
  injection transport layered underneath it for a specific UAV use case.
