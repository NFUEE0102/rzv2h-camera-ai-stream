#!/usr/bin/env python3
"""
uav_rx.py -- ground-station RX supervisor + operator console for the RZ/V2H
UAV video+detection link.
...
A human runs THIS. It:
  ...
  4. on START launches the local viewer (uav_view.py: GPU decode + SEI box
...
Run:  DISPLAY=:0 python3 uav_rx.py
"""
import sys
import os
import re
...
def enable_keepalive(sock, idle=3, intvl=2, cnt=3):
    """Turn on TCP keepalive so a hard drop (no FIN, e.g. the UAV flying out of
...
class RxSupervisor:
    def __init__(self):
        ...
        self.tx_host = None
        self.tx_state = "?"
        self.viewer = None               # subprocess.Popen of uav_view.py
    ...
    # =======================================================================
    # viewer engine (uav_view.py) control
    # =======================================================================
    ...
    # =======================================================================
    def run(self):
        self.evt.log("rx_boot", "uav_rx supervisor starting")
