#!/usr/bin/env python3
"""
stream_viewer.py -- RZ/V2H decoupled stream viewer + ADAPTIVE-BITRATE feedback (app_m5).
...
The board's capture_encoder runs the AIMD controller and live-sets omxh265enc bitrate.
...
a late joiner stays black until the next (never) IDR.
  ...
  DISPLAY=:0 python3 stream_viewer.py --fb-host 192.168.51.180 \
...
SEI_UUID = b"DETBOXSEI01_DRPI"          # 16 bytes, must match sei_box.h
...
FB_MAGIC = 0x4644424B   # 'FDBK' little-endian
FB_VER = 1
fb = {
...
def fb_tick():
    ...
    if fb["sock"] is not None and fb["dst"] is not None:
        ...
        try:
            ...
            pass
    # the [fb] line is parsed by the RX supervisor (ground_rx) for its telemetry CSV.
...
# ReconGate 進 LOST 時, 若 env RX_REQIDR=1, 向 RX_FB_HINT(host:port)送
# 1-byte UDP 提示。地面 fb sender(bpir3_fb.py)收到後於下一封 FbMsg 標 REQ_IDR、板端
# 補發 IDR。RX_FB_HINT 空(預設)=關。best-effort, LOST 持續時每秒重送一次。
...
# fb_tick 模式: 斷流時照跳), 每秒印一行穩定格式 [wait] 供人眼與 ground_rx.py status
...
def _gate_demo():
    """離線自檢(免 GStreamer): python3 -c 'import stream_viewer; stream_viewer._gate_demo()'。"""
    g = ReconGate()
    t = 100.0
    ...
    print("[stream_viewer] _gate_demo OK")
...
def apply_horizon(roll_deg):
    ...
    try:
        ...
        print("[stream_viewer] gltransformation set failed:", e, flush=True)
...
def main():
    ...
    ap.add_argument("--freeze", default="off", choices=["off", "on"],
                    help="D3 P0-1c: 凍幀+ReconGate 顯示狀態機(預設 off=零行為變更, "
                         ...
                         "另需 env RX_REQIDR=1 + RX_FB_HINT=host:port。")
    ...
    ap.add_argument("--record-file", default=None,
                    help="path for the recording (default ~/stream_release/RX/last_run.mkv)")
    a = ap.parse_args()
    ...
    rec_path = a.record_file or _os.path.join(
        _os.path.expanduser("~"), "stream_release", "RX", "last_run.mkv")
    if a.record:
        try:
    ...
    print("[stream_viewer] pipeline:", desc, flush=True)
    try:
        ...
        if use_gl:
            print("[stream_viewer] GL pipeline failed (%s) -- falling back to --no-gl" % e, flush=True)
            use_gl = False
    ...
    if fdsink is not None:
        try:
            ...
            print("[stream_viewer] fpsdisplaysink signal not available:", e, flush=True)
    ...
    if use_gl:
        ...
        if gltf is not None:
            ...
            try:
                ...
                print("[stream_viewer] gltransformation seed failed:", e, flush=True)
            print("[stream_viewer] horizon-lock GL rotate ON (sign=%+d)" % int(horizon["sign"]),
                  flush=True)
        else:
            print("[stream_viewer] WARN: gltransformation 'gltf' not found -- GL rotate off", flush=True)
    elif a.horizon == "imu":
        print("[stream_viewer] horizon-lock: GL rotate disabled (--no-gl); boxes still "
    ...
    if not a.no_sei:
        ...
        if hp is not None:
            ...
            if hp_src is not None:
                ...
                print("[stream_viewer] SEI box extraction ON (h265parse src probe)", flush=True)
    ...
    if dec is not None:
        ...
        if dec_src is not None:
            ...
            print("[stream_viewer] real display-fps counter ON (decoder src probe)",
                  flush=True)
        else:
            print("[stream_viewer] WARN: decoder src pad missing -- disp_fps unavailable",
                  flush=True)
    else:
        print("[stream_viewer] WARN: decoder element 'dec' not found -- disp_fps unavailable",
              flush=True)
    ...
    if a.fb_host:
        ...
        print("[stream_viewer] adaptive feedback -> %s:%d (1 Hz)" % (a.fb_host, a.fb_port), flush=True)
    ...
    if a.freeze == "on":
        ...
        # REQ_IDR 上行 hint: 額外需 env RX_REQIDR=1 且 RX_FB_HINT=host:port
        if os.environ.get("RX_REQIDR") == "1":
            hint = os.environ.get("RX_FB_HINT", "").strip()
            if hint and ":" in hint:
                h, _, p = hint.rpartition(":")
                try:
                    _reqidr["dst"] = (h, int(p))
                    _reqidr["sock"] = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                    print("[stream_viewer] REQ_IDR hint -> %s:%s (on ReconGate LOST)" % (h, p), flush=True)
                except Exception as e:
                    print("[stream_viewer] REQ_IDR hint disabled (bad RX_FB_HINT):", e, flush=True)
        GLib.timeout_add(200, gate_tick)   # 5 Hz 狀態機驅動
        print("[stream_viewer] freeze/ReconGate ON (hold<=%.1fs, red>%.1fs)"
    ...
    def on_msg(_b, m):
        if m.type == Gst.MessageType.ERROR:
            err, dbg = m.parse_error(); print("[stream_viewer] ERROR", err, dbg, flush=True); loop.quit()
        elif m.type == Gst.MessageType.EOS:
            print("[stream_viewer] EOS", flush=True); loop.quit()
    bus.connect("message", on_msg)

    if a.record:
        print("[stream_viewer] RECORDING last-run H.265 -> %s (overwrites each START)"
              % rec_path, flush=True)
    ...
    def _graceful_quit():
        ...
        shutdown["done"] = True
        if a.record:
            print("[stream_viewer] shutdown: sending EOS to finalize the recording...",
                  flush=True)
            try:
    ...
    print("[stream_viewer] PLAYING (rtp:%d bbox:%d dec:%s sink:%s rotate:%s horizon:%s gl:%s rec:%s)" %
